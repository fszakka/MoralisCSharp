...
namespace Parse
{
    ...
    public class ParseObject : IEnumerable<KeyValuePair<string, object>>, INotifyPropertyChanged
    {
        ...
        public IDictionary<string, object> EstimatedData { get; } = new Dictionary<string, object> { };
        ...
        protected virtual async Task SaveAsync(Task toAwait, CancellationToken cancellationToken)
        {
            ...
            var sessionToken = await Services.GetCurrentSessionTokenAsync();
            try
            {
                await Services.DeepSaveAsync(EstimatedData, sessionToken, cancellationToken);
                var state = await Services.ObjectController.SaveAsync(State, currentOperations, sessionToken, Services, cancellationToken);
                HandleSave(state);
            }
            catch (Exception e)
            {
